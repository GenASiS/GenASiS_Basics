!-- IncomingMessageArrayBigIntegerForm provides the concrete extension of 
!   MessageArrayTemplate for big ( long )integer datatype to handle receiving 
!   array of messages.

module IncomingMessageArrayBigInteger_Form

  use MPI
  use VariableManagement
  use MessagePassingBasics
  use Message_Template
  use IncomingMessageBigInteger_Form
!-- See FIXME below
!  use MessageArrayBigInteger_Form
  use MessageArray_Template 
 
  implicit none
  private

!-- FIXME: Bypass MessageArrayBigIntegerForm to avoid failed Finalization 
!   of polymorphic arrays with the Cray compiler
!  type, public, extends ( MessageArrayBigIntegerForm ) :: &
  type, public, extends ( MessageArrayTemplate ) :: &
    IncomingMessageArrayBigIntegerForm
      type ( IncomingMessageBigIntegerForm ), dimension ( : ), allocatable :: &
        Message
  contains
    procedure, public, pass :: &
      Initialize => InitializeAllocate_BI
    procedure, public, pass :: &
      Receive => Receive_BI
    final :: &
      Finalize_IMA_BI
  end type IncomingMessageArrayBigIntegerForm

contains


  subroutine InitializeAllocate_BI ( MA, C, Tag, Rank, nElements )

    class ( IncomingMessageArrayBigIntegerForm ), intent ( inout ), target :: &
      MA
    type ( CommunicatorForm ), intent ( in ), target :: &
      C
    integer ( KDI ), dimension ( : ), intent ( in ) :: &
      Tag, &
      Rank, &
      nElements

    integer ( KDI ) :: &
      nMessages
!-- See FIXME above
    integer ( KDI ) :: &
      iM    !-- iMessage
    type ( IncomingMessageBigIntegerForm ), pointer :: &
      M      

    nMessages = size ( Tag )

!-- See FIXME above
!     allocate ( IncomingMessageBigIntegerForm :: MA % Message ( nMessages ) )

!     call MA % MessageArrayBigIntegerForm % Initialize &
!            ( C, Tag, Rank, nElements )

    allocate ( MA % Message ( nMessages ) )

    MA % nMessages = size ( Tag )
    MA % MessageTemplate => MA % Message
    
    do iM = 1, MA % nMessages
      M => MA % Message ( iM )
      call M % Initialize ( C, Tag ( iM ), Rank ( iM ), nElements ( iM ) )
    end do

    nullify ( M )

  end subroutine InitializeAllocate_BI


  subroutine Receive_BI ( IMA )

    class ( IncomingMessageArrayBigIntegerForm ), intent ( inout ), target :: &
      IMA

    integer ( KDI ) :: &
      iM    !-- iMessage
    class ( MessageTemplate ), dimension ( : ), pointer :: &
      M

    !-- FIXME: The select type only works here on the first iteration,
    !          while a comparable structure in 
    !          MessageArrayForm % InitializeAllocate seems to work fine
!     do iM = 1, IMA % nMessages
!       select type ( Message => IMA % Message ( iM ) )
!         type is ( IncomingMessageForm )
!           call Message % Receive ( )
!       end select
!     end do

    M => IMA % Message 
    select type ( M )
    type is ( IncomingMessageBigIntegerForm )
      do iM = 1, IMA % nMessages
        call M ( iM ) % Receive ( )
      end do
    end select

    nullify ( M )

  end subroutine Receive_BI


  elemental subroutine Finalize_IMA_BI ( IMA )

    type ( IncomingMessageArrayBigIntegerForm ), intent ( inout ) :: &
      IMA

    !-- Trigger finalization of parent type

!-- See FIXME above
    if ( allocated ( IMA % Message ) ) deallocate ( IMA % Message )

  end subroutine Finalize_IMA_BI


end module IncomingMessageArrayBigInteger_Form
